...
export default async function SawaterPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const { projects, articles, faqs, reviews } = await getLocalizedServiceContent('sawater', locale);
              ...
              <div className="bg-gray-50 p-8 rounded-2xl border border-gray-100 shadow-sm">
                ...
                <div className="flex flex-wrap gap-3">
                  {(t.raw('seoArticle.keywordsList') as string[]).map((keyword: string, index: number) => (
                    <span key={`seo-k-${index}`} className="px-4 py-2 bg-white text-gray-700 text-sm font-medium rounded-full border border-gray-200 shadow-sm hover:border-accent hover:text-accent transition-colors cursor-default">
                      {keyword}
                    </span>
                  ))}
                </div>
